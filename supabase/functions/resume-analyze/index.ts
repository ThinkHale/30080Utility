// Ported from apps/resume-review/index.js (the root-level Azure Function,
// deployed as the "analyze" endpoint) — same system/user prompts and
// response shape, now calling Azure OpenAI via fetch instead of the
// @azure/openai SDK, running as a Supabase Edge Function.
...
Deno.serve(async (req: Request) => {
  ...
  const { jobDescription, positionNotes, resumeText, candidateName } = await req.json().catch(() => ({}));
